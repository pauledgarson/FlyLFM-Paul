...
% extract data for stim phases (2 sec before and 3 sec after On/Off set)
ONfirst=Brain((OnFirst-20):(OnFirst+29),:);
OFFfirst=Brain((OffFirst-20):(OffFirst+29),:);

ONsec=Brain((OnSec-20):(OnSec+29),:);
OFFsec=Brain((OffSec-20):(OffSec+29),:);

ONthird=Brain((OnThird-20):(OnThird+29),:);
OFFthird=Brain((OffThird-20):(OffThird+29),:);

StimResponses180=cat(3,ONfirst,OFFfirst,ONsec,OFFsec,ONthird,OFFthird);
...
StimPeaks180=[PeakONfirst;PeakOFFfirst;PeakONsec;PeakOFFsec;PeakONthird;PeakOFFthird];


%saves variable as new mat file with matching name
save(strcat(data(1:size(data,2)-6),'Peaks12.mat'),'StimResponses180','StimPeaks180');
